...
RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get install --assume-yes curl vim nano less mc sudo unoconv imagemagick apache2 mariadb-server subversion inkscape ghostscript antiword poppler-utils postfix libimage-exiftool-perl cron wget php php-dev php-gd php-mysql php-curl php-mbstring php-zip php-intl php-ldap libapache2-mod-php ffmpeg libgs-dev antiword zip python3-uno libreoffice-writer libreoffice-draw libreoffice-calc libreoffice-impress libreoffice-base-core libreoffice-core libreoffice-pdfimport libreoffice-avmedia-backend-gstreamer libreoffice-math libglu1-mesa
...
#scarico ed installo ResourceSpace versione 9.8
RUN mkdir /var/www/html.first \
    && cd /var/www/html.first \
	&& svn co http://svn.resourcespace.org/svn/rs/releases/9.8 . \
	&& mkdir filestore \
	&& chmod 777 filestore \
	&& chmod -R 777 include
